...
/**
 * Deploy DreamDuelEscrow to Somnia testnet.
 *
 * Reads the compiled artifact from contracts/out/, appends the ABI-encoded
 * constructor args, and broadcasts the create tx with the operator key.
 * Somnia's testnet uses an expensive gas schedule (cold storage ~1M gas), so a
 * generous explicit gas limit is passed (overshooting is safe — only consumed
 * gas is paid).
 *
 * Run:  npx tsx scripts/deploy-escrow.ts
...
async function main() {
  ...
  const artifactPath = path.resolve(
    process.cwd(),
    "contracts/out/DreamDuelEscrow.sol/DreamDuelEscrow.json",
  );
  const artifact = JSON.parse(fs.readFileSync(artifactPath, "utf8"));
  const initcode = artifact.bytecode.object as string;

  const args = encodeAbiParameters(parseAbiParameters("address,address,uint256"), [
  ...
  console.log(`DREAMDUEL ESCROW DEPLOYED at: ${receipt.contractAddress}`);
  console.log(`gasUsed=${receipt.gasUsed}`);

  // Write the address back into config for the app + wiring.
  const cfgPath = path.resolve(process.cwd(), "src/lib/ec/config.ts");
  let cfg = fs.readFileSync(cfgPath, "utf8");
  const old = cfg.match(/export const ESCROW_ADDRESS = "0x[0-9a-fA-F]{40}"/)?.[0];
  if (old) {
    cfg = cfg.replace(old, `export const ESCROW_ADDRESS = "${receipt.contractAddress}"`);
    fs.writeFileSync(cfgPath, cfg);
    console.log("updated ESCROW_ADDRESS in src/lib/ec/config.ts");
  }
}
